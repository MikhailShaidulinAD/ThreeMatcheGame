...
UCLASS()
class THREEMATCHGAME_API AGrid : public AActor
{
	GENERATED_BODY()
...
public:	
	...
	void OnMoveMade(EThreeMatchMoveType::Type MoveType);
	...
	void SetLastMove(EThreeMatchMoveType::Type MoveType);

	/** Tells what type of move the specified player made most recently. */
	EThreeMatchMoveType::Type GetLastMove();

	/** Gives point value per tile based on move type. Default is 100. */
	UFUNCTION(BlueprintNativeEvent, Category = Game)
	int32 GetScoreMultiplierForMove(EThreeMatchMoveType::Type LastMoveType);
	virtual int32 GetScoreMultiplierForMove_Implementation(EThreeMatchMoveType::Type LastMoveType);
...
private:
	...
	TMap<APlayerController*, EThreeMatchMoveType::Type> LastMoves;
