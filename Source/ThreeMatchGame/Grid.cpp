...
void AGrid::RespawnTiles()
{
	...
	{
		SetLastMove(EThreeMatchMoveType::MT_Combo);
	...
	else
	{
		if (IsUnwinnable())
		{
			if (AThreeMatchGameMode* GameMode = Cast<AThreeMatchGameMode>(UGameplayStatics::GetGameMode(this)))
			{
				GameMode->GameOver();
				return;
			}
		}
		UThreeMatchBlueprintFunctionLibrary::PauseGameTimer(this, false);
	}
}
...
TArray<ATile*> AGrid::GetExplosionList(ATile* A) const
{
	...
	if (AThreeMatchGameMode* GameMode = Cast<AThreeMatchGameMode>(UGameplayStatics::GetGameMode(A)))
	{
...
void AGrid::ExecuteMatch(const TArray<ATile*>& MatchingTiles)
{
	...
	UThreeMatchBlueprintFunctionLibrary::PauseGameTimer(this, true);
	...
	// Add score based on tile count.
	{
		if (AThreeMatchGameMode* GameMode = Cast<AThreeMatchGameMode>(UGameplayStatics::GetGameMode(this)))
		{
			EThreeMatchMoveType::Type MT = GetLastMove();
			int32 ScoreMult = GetScoreMultiplierForMove(MT);
			// Special results for certain move types.
			switch (MT)
			{
			case EThreeMatchMoveType::MT_Bomb:
			case EThreeMatchMoveType::MT_AllTheBombs:
				// Clear combo when bombing.
				GameMode->SetComboPower(0);
				break;
			case EThreeMatchMoveType::MT_Combo:
				// Power up combo!
...
void AGrid::OnSwapDisplayFinished(ATile* Tile)
{
	...
	if (SwappingTiles.Num() == 2)
	{
		...
		if (bPendingSwapMoveSuccess)
		{
			...
			{
				SetLastMove(EThreeMatchMoveType::MT_MoreTiles);
			}
			else
			{ 
				SetLastMove(EThreeMatchMoveType::MT_Standard);
			}
		...
		{
			SwappingTiles.Empty();
			OnMoveMade(EThreeMatchMoveType::MT_Failure);
		}
	}
}
...
void AGrid::OnTileWasSelected(ATile* NewSelectedTile)
{
	...
	if (FallingTiles.Num() || TilesBeingDestroyed.Num() || bPendingSwapMove || !UThreeMatchBlueprintFunctionLibrary::IsGameActive(this) || !NewSelectedTile)
	{
		return;
	}
		...
		if (NewSelectedTileType.Abilities.CanExplode())
		{
			...
			if (AThreeMatchGameMode* GameMode = Cast<AThreeMatchGameMode>(UGameplayStatics::GetGameMode(this)))
			{
				if (GameMode->GetComboPower() == GameMode->GetMaxComboPower())
				{
					...
					SetLastMove(EThreeMatchMoveType::MT_AllTheBombs);
			...
			if (TilesToDestroy.Num() == 0)
			{
				...
				SetLastMove(EThreeMatchMoveType::MT_Bomb);
...
void AGrid::SetLastMove(EThreeMatchMoveType::Type MoveType)
{
	if (APlayerController* PC = UThreeMatchBlueprintFunctionLibrary::GetLocalPlayerController(this))
	{
		...
		EThreeMatchMoveType::Type& LastMoveType = LastMoves.FindOrAdd(PC);
		LastMoveType = MoveType;
	}
}

EThreeMatchMoveType::Type AGrid::GetLastMove()
{
	...
	if (APlayerController* PC = UThreeMatchBlueprintFunctionLibrary::GetLocalPlayerController(this))
	{
		if (EThreeMatchMoveType::Type* MoveType = LastMoves.Find(PC))
		{
			return *MoveType;
		}
	}
	return EThreeMatchMoveType::MT_None;
}

int32 AGrid::GetScoreMultiplierForMove_Implementation(EThreeMatchMoveType::Type LastMoveType)
{
